"""
Load Companies Script for Docker Environment

This script runs inside the Docker container to load 5 real companies 
with maximum historical data available, including Lockheed Martin (LMT).

Run this with: docker-compose exec backend python load_companies_docker.py
"""
...
def main():
    ...
    logger.info("🚀 Starting company data loading process (Docker Environment)")
    logger.info(f"Loading {len(COMPANIES)} companies with maximum historical data...")
    logger.info(f"Companies: {', '.join(COMPANIES)}")
    
    # Get database session using the app's session management
    db = next(get_db())
    ...
    try:
        ...
        logger.info("The backend API is running at http://localhost:8000")
        logger.info("You can now:")
        logger.info("1. Access API endpoints at http://localhost:8000/api/companies/")
