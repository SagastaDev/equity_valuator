#!/usr/bin/env python3
"""
Script to initialize AlphaFinance Pro dummy data provider for testing field mapping functionality.
This script can be safely removed once real providers are configured.

Usage: python DummyProviders/init_dummy_provider02.py [DATABASE_URL]
"""
...
def load_mockup_data():
    """Load the MockupData02.json file"""
    mockup_file = Path(__file__).parent / "MockupData02.json"
    with open(mockup_file, 'r') as f:
        return json.load(f)

def init_dummy_provider(database_url: str):
    """Initialize AlphaFinance Pro dummy provider and sample data"""
    ...
    with SessionLocal() as db:
        ...
        db.commit()
        print("Successfully loaded AlphaFinance Pro dummy data into database")
...
def main():
    ...
    print(f"Initializing AlphaFinance Pro dummy provider data...")
    print(f"Database URL: {database_url}")
    print(f"Source data: {Path(__file__).parent / 'MockupData02.json'}")
    print("-" * 50)
    
    try:
        provider_id = init_dummy_provider(database_url)
        print("-" * 50)
        print("SUCCESS: AlphaFinance Pro dummy provider initialization completed successfully!")
