#!/usr/bin/env python3
"""
Script to initialize DataStream Financial dummy data provider for testing field mapping functionality.
This script can be safely removed once real providers are configured.

Usage: python DummyProviders/init_dummy_provider04.py [DATABASE_URL]
"""
...
def load_mockup_data():
    """Load the MockupData04.json file"""
    mockup_file = Path(__file__).parent / "MockupData04.json"
    with open(mockup_file, 'r') as f:
        return json.load(f)

def init_dummy_provider(database_url: str):
    """Initialize DataStream Financial dummy provider and sample data"""
    ...
    with SessionLocal() as db:
        ...
        if not dummy_provider:
            dummy_provider = Provider(
                name="DataStream Financial (Test)"
            )
        ...
        for report in mockup["reports"]:
            ...
            # Handle nested structure specific to DataStream format
            ...
            market_fields = {
                "price_per_share": report.get("price_per_share"),
        ...
        db.commit()
        print("Successfully loaded DataStream Financial dummy data into database")
...
def main():
    ...
    print(f"Initializing DataStream Financial dummy provider data...")
    print(f"Database URL: {database_url}")
    print(f"Source data: {Path(__file__).parent / 'MockupData04.json'}")
    print("-" * 50)
    
    try:
        provider_id = init_dummy_provider(database_url)
        print("-" * 50)
        print("SUCCESS: DataStream Financial dummy provider initialization completed successfully!")
