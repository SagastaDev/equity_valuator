#!/usr/bin/env python3
"""
Script to initialize MarketInsight Analytics dummy data provider for testing field mapping functionality.
This script can be safely removed once real providers are configured.

Usage: python DummyProviders/init_dummy_provider03.py [DATABASE_URL]
"""
...
def load_mockup_data():
    """Load the MockupData03.json file"""
    mockup_file = Path(__file__).parent / "MockupData03.json"
    with open(mockup_file, 'r') as f:
        return json.load(f)

def init_dummy_provider(database_url: str):
    """Initialize MarketInsight Analytics dummy provider and sample data"""
    ...
    with SessionLocal() as db:
        ...
        if not dummy_provider:
            dummy_provider = Provider(
                name="MarketInsight Analytics (Test)"
            )
        ...
        for report in mockup["reports"]:
            ...
            # Handle nested structure specific to MarketInsight format
            for main_section, data in report.items():
                if main_section in ["financials", "position", "cashflow"]:
            ...
            market_fields = {
                "stock_price": report.get("stock_price"),
        ...
        db.commit()
        print("Successfully loaded MarketInsight Analytics dummy data into database")
...
def main():
    ...
    print(f"Initializing MarketInsight Analytics dummy provider data...")
    print(f"Database URL: {database_url}")
    print(f"Source data: {Path(__file__).parent / 'MockupData03.json'}")
    print("-" * 50)
    
    try:
        provider_id = init_dummy_provider(database_url)
        print("-" * 50)
        print("SUCCESS: MarketInsight Analytics dummy provider initialization completed successfully!")
